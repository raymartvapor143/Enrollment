<?php 
  ...
  if (isset($_POST['updatestudent'])) {
  	...
  	$query = "UPDATE `gradeseven` SET `name`='$name',`roll`='$roll',`city`='$address',`mother`='$mother',`photo`='$photo',`middle`='$middle',`lastname`='$lastname',`bdate`='$bdate',`father`='$father',`gender`='$gender' WHERE `id`= $id";
  	if (mysqli_query($db_con,$query)) {
		...
		if (!empty($_FILES['photo']['name'])) {
  		...
  		header('Location: index.php?page=grade7');
  	}else{
  		header('Location: index.php?page=grade7&edit=error');
  	}
  }
?>
<h1 class="text-primary">
<a href="grade7.php" type="button" ><svg xmlns="http://www.w3.org/2000/svg" width="45" height="45" fill="currentColor" class="bi bi-arrow-left-square-fill" viewBox="0 0 16 16">
...
<i class="fas fa-user-plus"></i>  Edit Student Informations!<small class="text-warning"> Edit Student!</small></h1>
	...
	<?php
		if (isset($id)) {
			$query = "SELECT `id`, `name`, `roll`, `city`, `mother`, `photo`, `middle`, `lastname`, `father`, `bdate`, `gender`, `datetime` FROM `gradeseven` WHERE `id`=$id";
