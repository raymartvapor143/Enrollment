...
<?php 
  ...
  if (isset($_POST['addstudent'])) {
  	...
  	$query = "INSERT INTO `gradeseven`(`name`, `roll`, `city`, `mother`, `father`, `bdate`, `photo`, `middle`, `lastname`, `gender`) VALUES ('$name', '$roll', '$address', '$mother', '$father', '$bdate','$photo','$middle','$lastname','$gender');";
...
<h1 style="font-size: 50px; " class="text-primary"><i class="fas fa-users"></i>  Grade 7 / <small class="text-warning">Students List</small>   <button onclick="window.print();" class="btn" id="print-btn">
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
     <li class="breadcrumb-item" aria-current="page"><a href="grade7.php">Grade 7 </a></li>
     <li class="breadcrumb-item active" aria-current="page"> Section: QUISUMBING</li>
  </ol>
</nav>
...
<table class="table  table-striped table-hover table-bordered" id="data" style="margin-left: -34.5%; ">
  ...
  <tbody>
    <?php 
      $query=mysqli_query($db_con,'SELECT * FROM `gradeseven` ORDER BY `gradeseven`.`datetime` DESC;');
      $i=1;
      ...
      <tr>
        ...
        echo '<td>'.$i.'</td>
          ...
          <td>'.ucwords($result['mother']).'</td>
          <td>'.ucwords($result['father']).'</td>
          <td><img src="images/'.$result['photo'].'" height="50px">
          </td>
          <td>
            <a class="btn btn-xs btn-warning" href="index.php?page=editstudent7&id='.base64_encode($result['id']).'&photo='.base64_encode($result['photo']).'">
              <i class="fa fa-edit"></i></a>

             &nbsp; <a class="btn btn-xs btn-danger" onclick="javascript:confirmationDelete($(this));return false;" href="index.php?page=delete7&id='.base64_encode($result['id']).'&photo='.base64_encode($result['photo']).'">
